...
openai = ChatOpenAI(model_name="gpt-4", openai_api_key=os.getenv("OPENAI_KEY"))
...
def message_handler(body, say, event, logger, client):
    try:
        ...
        if re.search(r'<@.*?>.*has joined', text):
            #"looks like someone has joined!")
            user_id = re.search(r'<@(.*?)>', text).group(1)
            ...
            # "It looks like you're mentioning someone!")
            pass
